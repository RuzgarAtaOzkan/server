...
const subtle = () =>
  ...
  err('crypto.subtle must be defined, consider polyfill');
// prettier-ignore
...
const hash = (msg: TArg<Bytes>): TRet<Bytes> =>
  gh('sha256', abytes(msg, undefined, 'message'));
...
export const __TEST: TRet<{
  ...
  lift_x: (x: bigint): TRet<Point> =>
    Point.fromAffine({ x, y: lift_x(x) }) as TRet<Point>,
  extractK: (rand: TArg<Bytes>): TRet<{ rx: Bytes; k: bigint }> =>
    extractK(rand),
});
...
const bytesToNumBE = (b: TArg<Bytes>): bigint =>
  big('0x' + (bytesToHex(b) || '0'));
...
const numTo32b = (num: bigint): TRet<Bytes> =>
  hexToBytes(padh(arange(num, 0n, B256), L2));
...
const bits2int_modN = (bytes: TArg<Bytes>): bigint =>
  modN(bits2int(abytes(bytes)));
/**
...
const hashes = {
  ...
  hmacSha256: undefined as
    | undefined
    | ((key: TArg<Bytes>, message: TArg<Bytes>) => TRet<Bytes>),
...
const hmacDrbgAsync = async <T>(
  ...
  // exported slot can be replaced or unset at runtime.
  const h = (...b: TArg<Bytes[]>) =>
    gha('hmacSha256Async', k, concatBytes(v, ...b));
...
const getSharedSecret = (
  ...
  return Point.fromBytes(publicKeyB)
    .multiply(secretKeyToScalar(secretKeyA))
...
const challenge = (...args: TArg<Bytes[]>): bigint =>
...
const verifySchnorr = (
  ...
  m: TArg<Bytes>,
  p: TArg<Bytes>,
): boolean => _verifSchnorr(s, m, p, challenge) as boolean;
