...
// @ts-ignore
const captureTrace = (
  ...args: Parameters<typeof Error.captureStackTrace>
): void => {
  if (
...
const u8fr = (buf: ArrayLike<number>): TRet<Bytes> =>
  Uint8Array.from(buf) as TRet<Bytes>;
...
const checkDigest = (value: TArg<Bytes>): TRet<Bytes> =>
  abytes(value, 64, 'digest');
/**
...
const getExtendedPublicKeyAsync = (
  secretKey: TArg<Bytes>,
): Promise<TRet<ExtK>> => sha512a(abytes(secretKey, L)).then(hash2extK);
 ...
 * ```
 */
const getPublicKey = (priv: TArg<Bytes>): TRet<Bytes> =>
...
const hashFinishA = <T>(res: TArg<Finishable<T>>): Promise<TRet<T>> =>
  sha512a((res as Finishable<T>).hashable).then(
    (res as Finishable<T>).finish,
  ) as Promise<TRet<T>>;
const hashFinishS = <T>(res: TArg<Finishable<T>>): TRet<T> =>
  (res as Finishable<T>).finish(
    sha512s((res as Finishable<T>).hashable),
  ) as TRet<T>;
...
const verifyAsync = async (
  ...
  hashFinishA(_verify(signature, message, publicKey, opts));
/**
...
const utils: {
  ...
  getExtendedPublicKeyAsync:
    getExtendedPublicKeyAsync as typeof getExtendedPublicKeyAsync,
